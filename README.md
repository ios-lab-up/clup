...
- **Subida de documentos**: presigned **PUT** a R2 (`PutObjectCommand` + `getSignedUrl`, con `forcePathStyle: true`) — R2 no soporta presigned POST (`createPresignedPost` → 501). El servidor nunca recibe los bytes. Al enviar el formulario de inscripción, el servidor hace `HeadObject` contra R2 para confirmar tamaño/MIME reales antes de crear la inscripción.
...
- **Recordatorio de examen**: idempotente vía `ReminderLog` con constraint único `(examDateId, registrationId, reminderScheduleId)` — uno por envío programado, no uno solo por inscripción. El cron corre cada `REMINDER_CRON_INTERVAL_MINUTES` (5 min, `src/lib/constants.ts` + `docker/cron/crontab`, deben cambiar juntos) para tolerar reinicios/drift sin perder ninguna ventana — es seguro porque el endpoint es idempotente.
...
- **Settings › Emails**: activar/desactivar cada uno de los 5 correos que manda la plataforma (`EmailSetting`, `src/lib/mail/notification-settings.ts` — cada call site de `sendMail` en las Server Actions lo checa antes de enviar). El recordatorio de examen, además, admite **varios envíos programados** (`ReminderSchedule`: cantidad configurada por el admin como número + unidad — minutos/horas/días — guardada internamente en minutos): "2 días antes", "1 día antes", "5 minutos antes", etc., cada uno independiente y con su propio `ReminderLog`. `reminder-service.ts` recorre todos los `ReminderSchedule` activos en cada tick del cron; el copy del correo (`exam-reminder.ts`) ajusta el texto ("mañana", "en 2 días", "en 5 minutos"...) según `minutesBefore`.
...
`POST /api/cron/reminders` recorre todos los `ReminderSchedule` activos (Settings › Emails), busca por cada uno las inscripciones **aprobadas** cuyo examen cae en su ventana ("N minutos/horas/días antes"), envía el correo, y registra cada envío en `ReminderLog` (único por `examDateId`+`registrationId`+`reminderScheduleId`) — llamarlo de más nunca duplica envíos. Si `EmailSetting["EXAM_REMINDER"]` está desactivado, no manda nada. Requiere el header `x-cron-secret` con el valor de `CRON_SECRET`.

El cron corre cada `REMINDER_CRON_INTERVAL_MINUTES` minutos (`docker/cron/crontab`) — ese valor **tiene que calzar** con la constante del mismo nombre en `src/lib/constants.ts`, o las ventanas de envío quedan descuadradas (huecos o traslapes entre ticks).
