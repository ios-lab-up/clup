...
- **`ExamDate.passingScore`** (no `Exam.passingScore`): el mismo examen puede tener distinto passing score en fechas distintas.
- **`Profile.studentId`** (matrícula): no está en el spec original, pero es indispensable para poder resolver las filas del CSV de carga masiva de resultados (`student_id`). Se captura una sola vez en la primera inscripción del alumno.
