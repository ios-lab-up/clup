...
- **Catálogo `Faculty` / `Career`**: lista institucional editable en `/admin/catalogo`. Se siembra en la migración `*_catalog_and_onboarding` (data migration, generada desde `src/lib/catalog-data.ts` con `scripts/gen-catalog-sql.ts`) para que prod la tenga sin pasos manuales; después la DB es la fuente de verdad. `Faculty.isExternal` es el bucket "Docente / Externo" (sin carreras, usa el default del examen).
